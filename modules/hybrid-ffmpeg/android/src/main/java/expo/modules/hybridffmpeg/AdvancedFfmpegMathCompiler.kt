// START OF FILE: AdvancedFfmpegMathCompiler.kt
package expo.modules.hybridffmpeg

object AdvancedFfmpegMathCompiler {
    
    // কনভার্ট করে FFmpeg crop ম্যাথকে GLSL Fragment Shader ম্যাথে রূপান্তর করে
    fun compileCropMathToGLSL(ffmpegMath: String): String {
        var glsl = ffmpegMath.lowercase()
        
        // UV Coordinates এ 1.0 মানে পুরো স্ক্রিন
        glsl = glsl.replace("in_w", "1.0")
        glsl = glsl.replace("iw", "1.0")
        glsl = glsl.replace("in_h", "1.0")
        glsl = glsl.replace("ih", "1.0")
        
        // আউটপুট উইডথ/হাইট (Fragment shader-এ আগেই ক্যালকুলেট করে uCropW হিসেবে পাঠাতে হবে)
        glsl = glsl.replace("out_w", "uCropW")
        glsl = glsl.replace("ow", "uCropW")
        glsl = glsl.replace("out_h", "uCropH")
        glsl = glsl.replace("oh", "uCropH")
        
        // Time ভ্যারিয়েবল
        glsl = glsl.replace(Regex("\\bt\\b"), "uTime")
        
        // OpenGL Float ফিক্সিং (যেমন 2 কে 2.0 করা, যেন টাইপ এরর না আসে)
        glsl = glsl.replace(Regex("(?<![a-zA-Z0-9.])(\\d+)(?![a-zA-Z0-9.])")) { match ->
            "${match.value}.0"
        }
        return glsl
    }
    ...
    fun evaluateTimeVisibility(expression: String, timeSeconds: Double): Boolean {
        ...
        // Pattern 2: lt(mod(t,7),3.5)*gte(t,3)  (Very common in 2O, 2P, 3O files)
        ...
        return true // Default visible if logic parsing fails
    }
}